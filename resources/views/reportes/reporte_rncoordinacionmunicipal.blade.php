...
<body>
	...
	<h5 align="center">GOBIERNO AUTONOMO DEPARTAMENTAL DEL BENI<br>DIRECCION DE COORDINACION MUNICIPAL<br>DETALLE DE RESERVAS DE NOMBRES</h5>
    ...
    <table cellspacing="0" width="100%" align="center" border="1" style="font-size: 7pt">
      ...
      @foreach($reservanombres as $rncm)
      <?php $numeroitems++ ?>
      <?php $sumaReservanombre += $rncm->costoReserva; ?>
      <tr>
          <td style="width:15px;">{{$numeroitems}}</td>
          <td>{{$rncm->provincia}}</td>
          <td>{{$rncm->municipio}}</td>
          <td>{{$rncm->localidad}}</td>
          <td>{{$rncm->nombre_reserva}}</td>
          <td>{{$rncm->nombreSolicitante}}</td>
          <td>{{$rncm->numeroRecibo}}</td>
          <td>{{$rncm->fechainicio}}</td>
          <td>{{$rncm->fechafin}}</td>
          <td>{{$rncm->costoReserva}} Bs.</td>
      </tr>
      @endforeach
    </table>
  </div>
